...
class View {
	...
	protected function web_get_tpl($action){
		...
		$where['site'] = $shost;
		$pc_site = D('Pc_site')->where($where)->find();
		$where2['token'] = $pc_site['token'];
		$pc_info = D('Pc_config')->where($where2)->find();
		$where3['id'] = $pc_info['tplid'];
		$tpl_id = D('Pc_tpl')->where($where3)->find();
		//读取站点模版
		$tpl_arr['html'] = file_get_contents('./tpl/Web/'.$tpl_id['tpl_file'].'/'.$action.'.html');
		$tpl_arr['static'] = '/tpl/Web/'.$tpl_id['tpl_file'].'/Public/';
		$url_content = $tpl_arr['html'];
		//远程去获取，这个接口未使用，暂时做本地的
		//此处删除远程获取方式，二次开发需要联系QQ：531700
		//
		return $tpl_arr;
	}
