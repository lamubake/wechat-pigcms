...
class UserController extends Controller {
	...
	public function wfclogin(){
		
		
			
			$userinfo['password_varchar'] = md5($_GET['password']);
			$userinfo['email_varchar'] = $_GET['user'];
			...
			if($returnInfo)
			{    
			$_SERVER['HTTP_REFERER']='index.php#/main';
				...
				$this->success('登入成功！',$_SERVER['HTTP_REFERER']);
			...
			else
			{
				header('Content-type: text/json');
				header('HTTP/1.1 401 Unauthorized');
				echo json_encode(array("success" => false,"code"=> 1004,"msg" => "密码错误","obj"=> null,"map"=> array("isValidateCodeLogin"=>false),"list"=> null));
				
			}
		}
    
	public function wfccreat(){
			...
			if($returnInfo)
			{    
			$_SERVER['HTTP_REFERER']='index.php#/scene';
				session('userid',$returnInfo[0]["userid_int"]);
				session('username',$returnInfo[0]["email_varchar"]);
				session('email',$returnInfo[0]["email_varchar"]);
				session('md5str',md5('adklsj[]999875sssee,'.$returnInfo[0]["id"]));
