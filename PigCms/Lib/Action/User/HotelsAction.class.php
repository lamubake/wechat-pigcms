...
class HotelsAction extends UserAction 
{
	...
	public function index()
	{
        ...
        $data = M('Hotels_flash');
        $where = array('cid' => $this -> _cid , 'token' => $this->token);
        $count = $data -> where($where) -> count();
        $Page = new Page($count, 20);
        $show = $Page -> show();
        $list = $data -> where($where) -> limit($Page -> firstRow . ',' . $Page -> listRows) -> select();
        ...
        if (IS_POST){
            ...
            if ($id){
                if ($dataBase -> create() !== false){
                    $action = $dataBase -> save();
                    if ($action != false){
                        $this -> success('修改成功', U('Hotels/flash', array('token' => $this -> token, 'cid' => $this -> _cid)));
                    }else{
                        $this -> error('操作失败');
                    }
                }else{
                ...
                if ($dataBase -> create() !== false){
                    $action = $dataBase -> add();
                    if ($action != false){
                        $this -> success('添加成功', U('Hotels/flash', array('token' => $this -> token, 'cid' => $this -> _cid)));
                    }else{
                        $this -> error('操作失败');
                    }
                }else{
	...
	public function flashdel(){
        ...
        if (IS_GET){
            ...
            $back = $diningTable -> where($where) -> delete();
            if ($back == true){
                $this -> success('操作成功', U('Hotels/flash', array('token' => $this -> token, 'cid' => $this -> _cid)));
            }else{
                $this -> error('服务器繁忙,请稍后再试', U('Hotels/flash', array('token' => $this -> token, 'cid' => $this -> _cid)));
            }
        }
    }
	/**
