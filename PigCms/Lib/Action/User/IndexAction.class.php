...
class IndexAction extends UserAction{
	...
	public function index(){
		$this->assign('eqx_User',$eqx_User); //整合
...
// 		}
	...
	public function eqx(){
				...
				if ($b){
				  ...
				  $a=$db->save($date);
				  if($a){$this->success('帐号同步成功',U(MODULE_NAME.'/index'));}else{$this->error('帐号同步失败,请检测表字段是否正常',U(MODULE_NAME.'/index'));}
				}else{
                   $this->error('帐号同步失败,请检测总后台场景相关信息是否填写正确',U(MODULE_NAME.'/index'));
				}
	...
	public function reeqx(){
			    ...
			    if ($b !== false){
					  ...
					  $date['eqxpassword']=$users['password'];
					  $aa=$db->save($date);
				        if($aa){$this->success('帐号重新同步成功',U(MODULE_NAME.'/index'));}else{$this->error('帐号重新同步失败,请检测表字段是否正常',U(MODULE_NAME.'/index'));}
				 }else{
                       $this->error('帐号重新同步失败,请检测总后台场景相关信息是否填写正确',U(MODULE_NAME.'/index'));
				 }
