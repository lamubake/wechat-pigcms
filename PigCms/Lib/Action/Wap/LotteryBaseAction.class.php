	...
	protected function get_prize($Lottery,$wecha_id){
		...
		if (intval($prizetype) && $prizetype<7 && $prizetype>0){
			//中奖了 - 模板消息通知
			 ...
			 if($Lottery['type'] == 6){
				 $Research = M('research')->where(array('lid'=>$Lottery['id']))->find();
				 $href = $this->siteUrl.U('Wap/Research/index',array('token'=>$this->_get('token'),'wecha_id'=>$wecha_id,'reid'=>$Research['id']));
			 }else{
