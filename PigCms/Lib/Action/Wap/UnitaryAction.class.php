...
class UnitaryAction extends WapAction
{
	...
	public function _initialize()
	{
		...
		if (!in_array(ACTION_NAME, array("payReturn", "payend", "nopayover"))) {
		}
	}
	...
	public function cartajax()
	{
		...
		case "delorder":
			...
			foreach ($order_list2 as $vo2 ) {
				...
				$where_cart2["order_id"] = $vo2["imicms_id"];
			...
			if ($order_list != NULL) {
				foreach ($order_list as $vo ) {
					...
					$where_cart["order_id"] = $vo["imicms_id"];
	...
	public function dobuy()
	{
		...
		$update_order = $this->m_order->where(array("imicms_id" => $id_order))->save($save_order);
	...
	{
		$myorder = M("unitary_order")->where(array(
	"token"    => $this->token,
	"wecha_id" => $this->wecha_id,
	"paid"     => 1,
	"addtime"  => array("gt", 1444663403)
	))->order("addtime desc")->find();

		if ($myorder != NULL) {
			$where_cart["state"] = 1;
			$where_cart["token"] = $this->token;
			$where_cart["order_id"] = $myorder["imicms_id"];
			$cart_list = $this->m_cart->where($where_cart)->order("id")->select();

			foreach ($cart_list as $ck => $cv ) {
				$lucknum_cart_count = M("unitary_lucknum")->where(array("token" => $this->token, "order_id" => $myorder["imicms_id"], "cart_id" => $cv["id"]))->count();

				if ($lucknum_cart_count < $cv["count"]) {
					$this->redirect("Unitary/payend", array("token" => $this->token, "orderid" => $myorder["imicms_id"], "cart_i" => $ck, "cart_count" => $lucknum_cart_count));
					exit();
				}
			}

			$this->redirect("Unitary/payend", array("token" => $this->token, "orderid" => $myorder["imicms_id"]));
		}
	}
	...
	public function payReturn()
	{
		...
		if ($order) {
			if (1444663403 < $order["addtime"]) {
				$this->redirect("Unitary/payend", array("token" => $this->token, "orderid" => $order["imicms_id"]));
			}
			else {
				$this->redirect("Unitary/my", array("token" => $this->token));
			}
		}
		else {
			$this->error("支付未完成");
	...
	public function payend()
	{
		if ($_GET["dopayover"] == 1) {
			...
			$myorder = M("unitary_order")->where(array("token" => $this->token, "imicms_id" => intval($_GET["orderid"])))->find();
			...
			if ($myorder["addtime"] != 0) {
				...
				$where_cart["order_id"] = $_GET["orderid"];
				$cart_list = $this->m_cart->where($where_cart)->order("id")->select();
				$cart_list_count = count($cart_list);
				$this->assign("cart_list_count", $cart_list_count);
				$cart_i = ($_GET["cart_i"] ? intval($_GET["cart_i"]) : 0);

				if ($cart_i < $cart_list_count) {
					$unitary = M("unitary")->where(array("token" => $this->token, "id" => $cart_list[$cart_i]["unitary_id"]))->find();
					$unitary_price_array = S("UNITARY_PRICE_ARRAY_" . $this->token . "_" . $cart_list[$cart_i]["unitary_id"]);

					if ($unitary_price_array == NULL) {
						for ($j = 0; $j < $unitary["price"]; $j++) {
							$unitary_price_array[] = $j;
						}

						S("UNITARY_PRICE_ARRAY_" . $this->token . "_" . $cart_list[$cart_i]["unitary_id"], $unitary_price_array);
					}

					$cart_count = ($_GET["cart_count"] ? intval($_GET["cart_count"]) : 0);

					for ($i = $cart_count; $i < ($cart_count + 20); $i++) {
						if ($cart_list[$cart_i]["count"] <= $i) {
							$over = true;
						}
						else {
							$over = false;
							$lucknum_array = S("LUCKNUM_ARRAY_" . $this->token . "_" . $cart_list[$cart_i]["unitary_id"]);

							if ($lucknum_array == NULL) {
								$lucknum_array = array();
								$lucknum_list = M("unitary_lucknum")->where(array("token" => $this->token, "unitary_id" => $cart_list[$cart_i]["unitary_id"]))->field("lucknum")->select();
								...
								S("LUCKNUM_ARRAY_" . $this->token . "_" . $cart_list[$cart_i]["unitary_id"], $lucknum_array);
							}

							$add_lucknum["token"] = $this->token;
							$add_lucknum["wecha_id"] = $this->wecha_id;
							$add_lucknum["order_id"] = $_GET["orderid"];
							$add_lucknum["cart_id"] = $cart_list[$cart_i]["id"];
							$add_lucknum["unitary_id"] = $cart_list[$cart_i]["unitary_id"];
							list($s1, $s2) = explode(" ", microtime());
							$mtime = (double) sprintf("%.0f", (floatval($s1) + floatval($s2)) * 1000);
							$add_lucknum["addtime"] = $mtime;
							$lucknum_qc = array_diff($unitary_price_array, $lucknum_array);

							if (0 < count($lucknum_qc)) {
								sort($lucknum_qc);
								$mylucknum = $lucknum_qc[rand(0, count($lucknum_qc) - 1)];
								$lucknum_array[] = intval($mylucknum);
								$add_lucknum["lucknum"] = $mylucknum;
							}
							else {
								sort($lucknum_array);
								$add_lucknum["lucknum"] = $lucknum_array[count($lucknum_array) - 1] + 1;
								$lucknum_array[] = $add_lucknum["lucknum"];
							}

							S("LUCKNUM_ARRAY_" . $this->token . "_" . $cart_list[$cart_i]["unitary_id"], $lucknum_array);
							$add_lucknum["state"] = 0;
							$id_lucknum = M("unitary_lucknum")->add($add_lucknum);
						}
					}

					if ($over) {
						$params2["site"] = array("token" => $this->token, "from" => "一元购消息", "content" => "您的一元购有新的商品被购买" . $cart_list[$cart_i]["count"] . "人次，请注意查看。商品名称：" . $unitary["name"]);
						$error2 = MessageFactory::method($params2, "SiteMessage");
						$lucknum_array = S("LUCKNUM_ARRAY_" . $this->token . "_" . $cart_list[$cart_i]["unitary_id"]);
						$pay_count = count($lucknum_array);
						$save_unitary["proportion"] = ($pay_count / $unitary["price"]) * 100;
						if (($unitary["price"] <= $pay_count) && ($unitary["state"] != 2)) {
							$params["site"] = array("token" => $this->token, "from" => "一元购消息", "content" => "您的一元购有新的商品开奖，请注意查看。商品名称：" . $unitary["name"]);
							...
							$where_cart3["state"] = 0;
							$where_cart3["token"] = $this->token;
							$where_cart3["unitary_id"] = $cart_list[$cart_i]["unitary_id"];
							$del_cart3 = $this->m_cart->where($where_cart3)->delete();
							$save_unitary["proportion"] = 100;
							$update_unitary = $this->m_unitary->where(array("token" => $this->token, "id" => $cart_list[$cart_i]["unitary_id"]))->save($save_unitary);
							$where_lucknum2["unitary_id"] = $cart_list[$cart_i]["unitary_id"];
							$where_lucknum2["token"] = $this->token;
							$where_lucknum2["lucknum"] = $lucknum;
							$where_lucknum2["state"] = 0;
							$save_lucknum2["state"] = 1;
							...
							$find_lucknum2 = $this->m_lucknum->where($where_lucknum2)->find();
							$model = new templateNews();
							$model->sendTempMsg("TM00695", array("href" => $this->siteUrl . U("Wap/Unitary/goodswhere", array("token" => $this->token, "unitaryid" => $cart_list[$cart_i]["unitary_id"])), "wecha_id" => $find_lucknum2["wecha_id"], "title" => "一元夺宝中奖通知", "headinfo" => "恭喜您在一元夺宝中获得【" . $unitary["name"] . "】点击查看", "program" => $unitary["name"], "result" => date("Y年m月d日H时i分s秒"), "remark" => ""));
						}

						$update_unitary = $this->m_unitary->where(array("token" => $this->token, "id" => $cart_list[$cart_i]["unitary_id"]))->save($save_unitary);
						$cart_i++;
						$this->assign("cart_i", $cart_i);
						$this->assign("cart_count", 0);
					}
					else {
						$this->assign("cart_i", $cart_i);
						$cart_count = ($_GET["cart_count"] ? intval($_GET["cart_count"]) : 0);
						$this->assign("cart_count", $cart_count + 20);
						$this->assign("haisheng", $cart_list[$cart_i]["count"] - $cart_count);
					}
				}
				else {
					M("unitary_order")->where(array("token" => $this->token, "imicms_id" => intval($_GET["orderid"])))->save(array("addtime" => 0));
					$dopayover = 1;
				}

				$this->assign("dopayover", $dopayover);
			}
			else {
				$this->redirect("Unitary/index", array("token" => $this->token));
			}
		}
