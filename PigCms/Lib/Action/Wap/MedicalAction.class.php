...
$info[9]['name'] = $setIndex['menu10'];
    ...
    public function registered(){
		...
		if(!empty($user)) $reser = array_merge($reser,$user);
    ...
    public function add(){
       if(IS_POST){
        ...
        //打印挂号单
//         $params['printer'] = array();
//         $params['printer']['token'] = $token;
//         $params['printer']['companyid'] = 0;
//         $params['printer']['type'] = 'Medical';
//         $params['printer']['content'] = $this->content_layout($da);
//         $params['printer']['paid'] = $resdata['price'] > 0 ? 0 : 1;
//         $params['printer']['qr'] = '';
//         MessageFactory::method($params, array('PrinterMessage'));

		$op = new orderPrint();
		$paid = $resdata['price'] > 0 ? 0 : 1;
        ...
        if ($resdata['price'] > 0) {
            ...
            //$this->redirect('Alipay/pay', array('from' => 'Medical','orderName'=>'预约挂号费用', 'price'=>$resdata['price'],'token'=>$da['token'],'orderid'=>$da['orderid'], 'wecha_id'=>$da['wecha_id'],'type'=>$da['type'],'rid'=>$da['rid'], 'id'=> $insertdata), 3, '您好,准备跳转到支付页面,请不要重复刷新页面,请耐心等待...');
        } else {
            if (!empty($insertdata)) {
    ...
    public function payReturn(){
       ...
       if($checkOrder){//如果订单存在
            if ($checkOrder['paid'] == '1') { //支付成功,发信息,跳转到订单别表页面
				...
				//给商家发站内信
				$params = array();
				$params['site'] = array('token'=>$checkOrder['token'], 'from'=>'微医疗消息','content'=>"您的会员 {$checkOrder['truename']},已经预约了{$checkOrder['orderName']} 并付款成功,金额为{$checkOrder['price']},订单号为{$checkOrder['orderid']}");
...
// 				$params['printer']['companyid'] = 0;
// 				$params['printer']['type'] = 'Medical';
// 				$params['printer']['content'] = $this->content_layout($checkOrder);
// 				$params['printer']['paid'] = 1;
// 				$params['printer']['qr'] = '';
// 				MessageFactory::method($params, array('PrinterMessage'));
